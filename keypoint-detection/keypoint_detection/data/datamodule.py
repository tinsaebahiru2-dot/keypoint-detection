...
from keypoint_detection.data.coco_topdown_dataset import COCOTopDownKeypointsDataset
...
class KeypointsDataModule(pl.LightningDataModule):
    ...
    def add_argparse_args(parent_parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        ...
        parser.add_argument("--use_topdown", dest="use_topdown", default=False, action="store_true")
        parser.add_argument("--crop_width", type=int, default=256)
        parser.add_argument("--crop_height", type=int, default=256)
        parser.add_argument("--bbox_scale", type=float, default=1.25)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if json_dataset_path:
            if kwargs.get("use_topdown", False):
                self.train_dataset = COCOTopDownKeypointsDataset(
        ...
        if json_validation_dataset_path:
            if kwargs.get("use_topdown", False):
                self.validation_dataset = COCOTopDownKeypointsDataset(
        ...
        if json_test_dataset_path:
            if kwargs.get("use_topdown", False):
                self.test_dataset = COCOTopDownKeypointsDataset(
                    json_test_dataset_path, keypoint_channel_configuration, **kwargs
                )
            else:
